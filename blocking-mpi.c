...
///pisis na bgainei arkibis i dieresi npoints/size
    ...
    //omp_set_num_threads(omp_get_num_procs());///mporoume na kanoume xrisi tou ulimit gia na doume mexri posa thread mporoume na sikosoume
...
///kanw enan pinaka dist2 pou periexei tis apostaseis gia ka8e diergasia me ta kainouria simia pou ir8an
    ...
    ///arxikopiisi tou distance(epeidi einai sum prepei na arxikopoi8ei me 0)
    #pragma omp parallel
    {
...
///edw pernoyme tis apostaseis ka8e simiou apo to block simiwn poy exei ka8e diergasia
        ...
        ///kanouome sort ton dist kai ton pinaka_8esewn me basi ton dist
...
///pernw tous k_nn protous tou pinaka-8esewn afou ton ekana sort ara exw tous kontinoterous geitones
        #pragma omp parallel for schedule(static)
        for(i=0; i<npoints/size; i++)
        {
            for(j=0; j<k_nn; j++)
...
///telos protou iteration(se auto to simio exw tous 30 kontinoterous geitones twn prwtwn simiwn ka8e diergasias)
        ...
        for(x=0; x<size-1; x++)///size -1 logika
        {
///topologia daktiliou
///oles oi diergasies ektos tou rank=0 kanoun rcv
            if (rank != 0)
            {
                MPI_Recv(*send_recv,(npoints/size)*diastaseis, MPI_DOUBLE,rank - 1, tag, MPI_COMM_WORLD,&status);
                MPI_Recv(*pinakas_8esewn_sendrecv,(npoints/size)*(npoints/size), MPI_INT,rank - 1, tag+1, MPI_COMM_WORLD,&status);
            }

///oles oi diergasies kanoun send
            MPI_Send(&send2[0][0],(npoints/size)*diastaseis, MPI_DOUBLE, (rank + 1) % size,tag,MPI_COMM_WORLD);
            MPI_Send(&pinakas_8esewn_send[0][0],(npoints/size)*(npoints/size), MPI_INT, (rank + 1) % size,tag+1,MPI_COMM_WORLD);

///twra pleon mporei na kanei rcv kai h rank=0
///metaferoume ta dedomena me auton ton tropo gia na min exoyme deadlock
            if (rank == 0)
            {
                MPI_Recv(*send_recv,(npoints/size)*diastaseis, MPI_DOUBLE,size - 1, tag, MPI_COMM_WORLD,&status);
                MPI_Recv(*pinakas_8esewn_sendrecv,(npoints/size)*(npoints/size), MPI_INT,size - 1, tag+1, MPI_COMM_WORLD,&status);
            }
